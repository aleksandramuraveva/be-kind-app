...
// Thunks
export const searchFriends = createAsyncThunk('friends/searchFriends', async (searchTerm: string) => {
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/users/search?term=${searchTerm}`, {
    headers: {
...
export const addFriend = createAsyncThunk('friends/addFriend', async (friendUniqueTag: string) => {
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/friends/add`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${localStorage.getItem('token')}`,
    },
    body: JSON.stringify({ userId: Number(localStorage.getItem('userId')), friendUniqueTag }),
  });
  const data = await response.json();
  return data;
});
...
export const fetchFriends = createAsyncThunk('friends/fetchFriends', async (userId: number) => {
  const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/friends/${userId}`, {
    headers: {
      'Authorization': `Bearer ${localStorage.getItem('token')}`,
    },
  });
...
const friendsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(searchFriends.fulfilled, (state, action: PayloadAction<Friend[]>) => {
      ...
      .addCase(fetchFriendDeeds.fulfilled, (state, action: PayloadAction<FriendDeed>) => {
        state.status = 'succeeded';
        const index = state.friendDeeds.findIndex(fd => fd.friendId === action.payload.friendId);
        if (index !== -1) {
      ...
      .addCase(fetchFriends.fulfilled, (state, action: PayloadAction<Friend[]>) => { // Handle fetching friends
        state.status = 'succeeded';
